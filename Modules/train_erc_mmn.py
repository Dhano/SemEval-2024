...
def train(model, epochs):
    ...
    for epoch in tqdm.tqdm(range(epochs)):
        ...
        for i_batch, sample_batched in tqdm.tqdm(enumerate(data_iter_train)):
        ...
        avg_loss /= len(data_iter_train)
        print("Average Loss = ",avg_loss)        
        f1_1,v_loss = validate(model,epoch)
...
def validate(model,epoch):
    ...
    with torch.no_grad():
      ...
      for i_batch, sample_batched in tqdm.tqdm(enumerate(data_iter_test)):
...
print(len(data_iter_train),len(data_iter_test))
model = train(model, epochs = 100)


# model_path = '../Models/sim_model.pkl'

# # Save the model using pickle
# with open(model_path, 'rb') as file:
#     loaded_model = pickle.load(file)

# print(loaded_model(1,1))