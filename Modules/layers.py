...
class SimNetwork(nn.Module):
    ...
    def forward(self, input1, input2):
        ...
        vector1 = input1.reshape(-1)
        vector2 = input2.reshape(-1)

        

        
        # if type(vector1) == torch.Tensor:
        #     vector1 = vector1.detach().cpu().numpy()
        # if type(vector2) == torch.Tensor:
        #     vector2 = vector2.detach().cpu().numpy()
        
        cosine_sim = torch.cosine_similarity(vector1.reshape(1, -1), vector2.reshape(1, -1))
        
        #convert to tensor
        #cosine_sim=cosine_sim.to(torch.DoubleTensor).to(torch.device("cuda:2"))  
        #cosine_sim = torch.from_numpy(cosine_sim)
        # print("Shape of cosine sim is")
        # print(type(cosine_sim))
        # cosine_sim = cosine_sim.float()
        fc1_out = self.fc1(cosine_sim.float())    
...
class myTransformerEncoder(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, dp=0, bd=False):
        ...
        #     self.sim_network =  pickle.load(file)

        self.sim_network = SimNetwork(1)
        print("Sim network is Active.........................................")
    ...
    def forward(self, x):
        ...
        zero_row = np.zeros((1, x.shape[1], x.shape[2]))
        shifted_matrix = np.vstack((zero_row, x[:-1, :].cpu().detach().numpy()))
        shifted_matrix = torch.tensor(shifted_matrix).to(x.device)
        sim_shift= self.sim_network(shifted_matrix, x)
        # Pass the input through the stack of self-attention layers
        for layer in self.layers:  
            
            x = layer(x)
            #print(x.shape)
            #print("Shape of sim shoft is")
            #print(sim_shift.shape)
            x =(1-sim_shift)*x
            #print("After adding the sim..........")
            #print(x.shape)
        
        return x, None


class myTransformerEncoder2(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, dp=0, bd=False):
        super(myTransformerEncoder2, self).__init__()
...
class interact(nn.Module):
    def __init__(self,hidden_dim,weight_matrix,utt2idx):
        ...
        self.rnnD = myTransformerEncoder2(embedding_dim, hidden_dim,2)   #Dialogue
